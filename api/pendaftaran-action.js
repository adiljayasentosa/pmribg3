...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Nomor Induk dibuat otomatis dari prefix tetap + tanggal lahir (DDMMYY) + urutan 3 digit.
    // Format: 270124 + DDMMYY + NNN. Contoh: 270124280610001.
    ...
    // Gunakan counter baru agar reset urutan tidak mewarisi counter lama.
    // Counter ini dimulai dari 0, sehingga anggota pertama mendapat 001.
    const counterRef = db.collection('system_counters').doc('nomor_induk_anggota_reset_001');
    const anggotaRef = db.collection('anggota').doc();
    let nomorInduk = '';

    await db.runTransaction(async tx => {
      const counterSnap = await tx.get(counterRef);
      let nextNumber = counterSnap.exists ? Number(counterSnap.data().lastNumber || 0) + 1 : 1;

      if (!Number.isInteger(nextNumber) || nextNumber < 1) nextNumber = 1;
      if (nextNumber > 999) throw new Error('Nomor urut anggota sudah mencapai batas 999.');

      // Jika nomor hasil reset ternyata sudah pernah dipakai, lanjutkan ke nomor
      // kosong berikutnya agar data lama tidak tertimpa.
      while (nextNumber <= 999) {
        const nomorUrut = String(nextNumber).padStart(3, '0');
        const kandidat = `270124${kodeTanggalLahir}${nomorUrut}`;
        const existingNi = await tx.get(
          db.collection('anggota').where('nomorInduk', '==', kandidat).limit(1)
        );
        if (existingNi.empty) {
          nomorInduk = kandidat;
          break;
      ...
      if (!nomorInduk) throw new Error('Nomor urut anggota sudah mencapai batas 999.');
