  ...
  document.getElementById('btn-kta-generate-all')?.addEventListener('click', async()=>{
    ...
    try {
      for (let i=0;i<ready.length;i++) {
        const a=ready[i];
        const canvas=await buildKtaCanvas(a,`${location.origin}/k/${encodeURIComponent(a.ktaToken)}`);
        const blob=await new Promise(r=>canvas.toBlob(r,'image/png'));
        zip.file(`KTA-${safeFile(a.nomorInduk||a.nama)}-depan.png`,blob);
        const backCanvas=await buildKtaBackCanvas(a,`${location.origin}/k/${encodeURIComponent(a.ktaToken)}`);
...
async function previewKta(a, fromAll=false) {
  ...
  const target = `${location.origin}/k/${encodeURIComponent(token)}`;
...
async function buildKtaCanvas(a,target) {
  ...
  // Full inner photo area. The old height (532px) left a 6–7px strip of the template's green landscape visible below the photo.
  const px=245,py=438,pw=503,ph=539; ctx.save();ctx.beginPath();ctx.rect(px,py,pw,ph);ctx.clip(); const photo=await tryLoadKtaPhoto(a.foto); if(photo){const scale=Math.max(pw/photo.width,ph/photo.height),w=photo.width*scale,h=photo.height*scale;ctx.drawImage(photo,px+(pw-w)/2,py+(ph-h)/2,w,h);}else{ctx.fillStyle='#d9edf7';ctx.fillRect(px,py,pw,ph);ctx.fillStyle='#fff';ctx.font='bold 54px Inter, Arial';ctx.textAlign='center';ctx.fillText('FOTO',px+pw/2,py+ph/2);ctx.font='28px Inter, Arial';ctx.fillText('Belum tersedia',px+pw/2,py+ph/2+48);}ctx.restore();
...
function _initDashboard(user) {
  ...
  /* ── Portal khusus role ANGGOTA ──
     ...
     KTA publik tetap melalui /k/<token> dan tetap terhubung ke token KTA. */
